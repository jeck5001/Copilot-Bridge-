...
func TestModelsAdvertiseContextAndReasoning(t *testing.T) {
	...
	wantContext := map[string]float64{
		"gpt-5.2": 400000, "gpt-5.2-reasoning": 400000,
		"gpt-5.3": 400000,
		"gpt-5.4": 1050000, "gpt-5.4-reasoning": 1050000,
		"gpt-5.5": 128000, "gpt-5.5-reasoning": 128000,
		"gpt-5.6-sol": 128000, "gpt-5.6-reasoning": 128000,
		"gpt-5.6-terra": 128000, "gpt-5.6-luna": 128000,
		"claude-sonnet": 200000, "claude-sonnet-reasoning": 200000,
	}
	for _, m := range body.Data {
		id, _ := m["id"].(string)
		if got, ok := wantContext[id]; !ok || m["context_window"].(float64) != got {
			t.Fatalf("wrong context for %q: %#v", id, m)
		}
		delete(wantContext, id)
		...
		if id != "gpt-5.3" && caps["reasoning"] != true {
			t.Fatalf("reasoning not advertised: %#v", m)
		}
		if id == "gpt-5.3" && caps["reasoning"] != false {
			t.Fatalf("unreliable gpt-5.3 reasoning advertised: %#v", m)
		}
	}
	if len(wantContext) != 0 {
		t.Fatalf("models missing from catalog: %#v", wantContext)
	}
}

func TestModelSpecificLimitsMatchClientCatalog(t *testing.T) {
	cases := []struct {
		model                  string
		context, output, input int
	}{
		{"gpt-5.3", 400000, 128000, 272000},
		{"gpt-5.6-sol", 128000, 16384, 96000},
		{"gpt-5.6", 128000, 16384, 96000},
		{"claude-sonnet", 200000, 64000, 136000},
	}
	for _, tc := range cases {
		got := modelLimitsFor(tc.model)
		if got.ContextWindow != tc.context || got.MaxOutputTokens != tc.output || got.MaxInputTokens != tc.input {
			t.Fatalf("%s limits=%+v", tc.model, got)
		}
	}
}
...
func TestReasoningEffortRouting(t *testing.T) {
	...
	if _, err := reasoningTone("gpt-5.3", "high"); err == nil {
		t.Fatal("unreliable gpt-5.3 reasoning tone accepted")
	}
}

func TestUnsupportedModelIsRejectedBeforeUpstream(t *testing.T) {
	s := &Server{}
	r := httptest.NewRequest("POST", "/v1/chat/completions", strings.NewReader(`{"model":"gpt-5.3-reasoning","messages":[{"role":"user","content":"hello"}]}`))
	w := httptest.NewRecorder()
	s.openaiChat(w, r)
	if w.Code != 400 || !strings.Contains(w.Body.String(), "unsupported model") {
