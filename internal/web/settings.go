...
func defaultRuntimeSettings() runtimeSettings {
	return runtimeSettings{
		MaxToolCallsPerTurn: envInt("M365_MAX_TOOL_CALLS_PER_TURN", 4), MaxToolRounds: envInt("M365_MAX_TOOL_ROUNDS", 64),
...
func settingsPath() string {
	...
	h, _ := os.UserHomeDir()
	return filepath.Join(h, ".config", "m365-gateway", "settings.json")
}
...
func openSettingsStore() (*settingsStore, error) {
	...
	if b, e := os.ReadFile(s.path); e == nil {
		...
		return nil, fmt.Errorf("read settings: %w", e)
	}
	if err := validateSettings(s.v); err != nil {
...
func validateSettings(v runtimeSettings) error {
	...
	for name, p := range map[string]string{
		"debugLogPath": v.DebugLogPath, "configPath": v.ConfigPath,
		"tokenCachePath": v.TokenCachePath, "sessionCachePath": v.SessionCachePath,
	} {
		...
		if previous, ok := seen[clean]; ok {
			return fmt.Errorf("%s 与 %s 不能使用同一路径", name, previous)
		}
		seen[clean] = name
	}
	return nil
...
var runtimeSettingEnvironment = map[string]string{
	"maxToolCallsPerTurn": "M365_MAX_TOOL_CALLS_PER_TURN",
	"maxToolRounds":       "M365_MAX_TOOL_ROUNDS",
	"contextWindow":       "M365_CONTEXT_WINDOW",
	"maxOutputTokens":     "M365_MAX_OUTPUT_TOKENS",
	"chatTimeoutSeconds":  "M365_CHAT_TIMEOUT_SECONDS",
	"imageTimeoutSeconds": "M365_IMAGE_TIMEOUT_SECONDS",
	"logLevel":            "M365_LOG_LEVEL",
	"debugLogPath":        "M365_DEBUG_LOG",
	"listenAddress":       "M365_LISTEN",
	"configPath":          "M365_CONFIG",
	"tokenCachePath":      "M365_TOKEN_CACHE",
	"sessionCachePath":    "M365_SESSION_CACHE",
...
func environmentOverriddenSettings() []string {
	overridden := make([]string, 0, len(runtimeSettingEnvironment))
	for setting, environment := range runtimeSettingEnvironment {
		if _, ok := os.LookupEnv(environment); ok {
			overridden = append(overridden, setting)
		}
	}
...
func (s *Server) adminSettings(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodGet:
		jsonOut(w, map[string]any{
			...
			"restartRequiredFields": []string{"listenAddress", "configPath", "tokenCachePath", "sessionCachePath", "clientId", "authority", "redirectUri", "scope", "debugLogPath"},
			"environmentOverrides":  environmentOverriddenSettings(),
		})
	case http.MethodPut:
...
func effectiveToolCallLimit(requestedParallel *bool, s *settingsStore) int {
	if requestedParallel != nil && !*requestedParallel {
		return 1
	}
	return configuredToolCallLimit(s)
}
