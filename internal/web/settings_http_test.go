...
func TestAdminSettingsReportsEnvironmentOverrides(t *testing.T) {
	t.Setenv("M365_CHAT_TIMEOUT_SECONDS", "123")
	...
	joined := strings.Join(response.EnvironmentOverrides, ",")
	if !strings.Contains(joined, "chatTimeoutSeconds") || !strings.Contains(joined, "logLevel") {
		t.Fatalf("environmentOverrides=%v", response.EnvironmentOverrides)
	}
}
