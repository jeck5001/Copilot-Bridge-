...
func TestResponsesNonStreamPreservesLengthAsIncomplete(t *testing.T) {
	source := map[string]any{"choices": []any{map[string]any{
		"finish_reason": "length",
		"message":       map[string]any{"role": "assistant", "content": "partial"},
	}}}
	rr := httptest.NewRecorder()
	writeResponsesResult(rr, "m", false, source)
	var body map[string]any
	if err := json.Unmarshal(rr.Body.Bytes(), &body); err != nil {
		t.Fatal(err)
	}
	if body["status"] != "incomplete" {
		t.Fatalf("status=%v body=%s", body["status"], rr.Body.String())
	}
...
func TestResponsesNonStreamEmptyLengthIsIncomplete(t *testing.T) {
	source := map[string]any{"choices": []any{map[string]any{
		"finish_reason": "length",
	...
	if _, exists := body["completed_at"]; exists {
		t.Fatalf("incomplete response exposed completed_at: %s", rr.Body.String())
	}
}
